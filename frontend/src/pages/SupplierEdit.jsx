...
import { getSupplierEditMeta, updateSupplier } from '../api'
...
export default function SupplierEdit() {
  ...
  const load = () => getSupplierEditMeta(id)
    .then((data) => {
      setMeta(data)
  ...
  const handleSubmit = (e) => {
    ...
    updateSupplier(id, {
      ...
      .then(() => { showToast('供应商已保存', 'success'); navigate('/suppliers') })
    ...
    <div className="space-y-4">
      <div>
        <nav className="text-sm text-gray-500 mb-2">
          ...
          <span className="text-gray-800">编辑供应商</span>
        </nav>
        <h1 className="text-2xl font-bold text-gray-800">编辑供应商</h1>
        <p className="text-gray-600 text-base mt-1">修改供应商信息后保存</p>
      </div>
