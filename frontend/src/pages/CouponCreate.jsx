...
import { getCouponCreateMeta, createCoupon } from '../api'
...
export default function CouponCreate() {
  ...
  const [form, setForm] = useState({
    ...
    valid_until: formatDateForInput(new Date(Date.now() + 30 * 24 * 3600 * 1000)),
    total_quantity: 100,
    category_ids: [],
    status: 'active',
    description: '',
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    if (!form.code.trim()) { showToast('请输入券码'); return }
    ...
    if (!form.total_quantity || Number(form.total_quantity) < 1) { showToast('发行总量至少为 1'); return }
    ...
    createCoupon(payload)
      .then(() => { showToast('优惠券已创建', 'success'); navigate('/coupons') })
      .catch((e) => showToast(e.message))
      .finally(() => setSubmitting(false))
  }

  if (loading && categories.length === 0) return <div className="flex justify-center py-12"><div className="animate-spin rounded-full h-10 w-10 border-2 border-primary border-t-transparent" /></div>
    ...
    <div className="space-y-4">
      <div>
        <nav className="text-sm text-gray-500 mb-2">
          ...
          <span className="text-gray-800">新增优惠券</span>
        </nav>
        <h1 className="text-2xl font-bold text-gray-800">新增优惠券</h1>
        <p className="text-gray-600 text-base mt-1">配置优惠券参数，支持满减券和折扣券</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-md border-2 border-gray-200 p-6 sm:p-8 max-w-3xl space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <div>
            <label className="block text-base font-semibold text-gray-800 mb-1.5">券码 <span className="text-red-500">*</span></label>
            <input
              type="text"
              value={form.code}
              onChange={(e) => update('code', e.target.value)}
              placeholder="如：SAVE100"
              maxLength={64}
            ...
            />
            <p className="text-xs text-gray-400 mt-1">英文、数字或组合，建议大写，全局唯一</p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <div>
            ...
            <input
              ...
              placeholder={form.type === 'fixed' ? '如：20.00' : '如：85 表示八五折'}
              className="block w-full rounded-lg border-2 border-gray-300 bg-white px-3 py-2.5 text-base text-gray-800 placeholder-gray-400 focus:border-primary focus:ring-2 focus:ring-primary/20 focus:outline-none"
            />
            {form.type === 'percent' && <p className="text-xs text-gray-400 mt-1">输入 85 表示八五折，100 表示免费</p>}
          </div>
        </div>
        ...
        <div>
          ...
          <input
            ...
            placeholder="0 表示无门槛，如：100.00 表示满100可用"
            className="block w-full rounded-lg border-2 border-gray-300 bg-white px-3 py-2.5 text-base text-gray-800 placeholder-gray-400 focus:border-primary focus:ring-2 focus:ring-primary/20 focus:outline-none"
          />
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <div>
            <label className="block text-base font-semibold text-gray-800 mb-1.5">发行总量 <span className="text-red-500">*</span></label>
            <input
              type="number"
              min="1"
              step="1"
          ...
          <div>
            <label className="block text-base font-semibold text-gray-800 mb-1.5">初始状态</label>
            <select
              value={form.status}
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex gap-2">
              ...
              <button type="button" onClick={clearCategorySelection} className="text-xs text-gray-500 hover:underline">清空（表示全部适用）</button>
            </div>
          </div>
          <p className="text-xs text-gray-400 mb-2">不选择任何分类 = 全部商品可用；选择后仅选中的分类可使用</p>
          {categories.length === 0 ? (
        ...
        <div>
          <label className="block text-base font-semibold text-gray-800 mb-1.5">描述（选填）</label>
          <textarea
            rows={3}
            value={form.description}
            onChange={(e) => update('description', e.target.value)}
            placeholder="可选的补充说明，不影响逻辑"
            maxLength={500}
            className="block w-full rounded-lg border-2 border-gray-300 bg-white px-3 py-2.5 text-base text-gray-800 placeholder-gray-400 focus:border-primary focus:ring-2 focus:ring-primary/20 focus:outline-none"
          />
        </div>
